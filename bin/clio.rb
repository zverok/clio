...
opts = Slop.parse(:help => true){
    ...
    on :imagesonly,
        "Загружать только картинки (не пытаться обновить фид)"
    on :files,
        "Загружать вложенные файлы (музыку). По умолчанию НЕ БУДУТ загружены"
    on :noconvert,
        "Не конвертировать в HTML (только скачать/построить индексы)"

    on :depth=,
        "Глубина загрузки (количество новых записей); по умолчанию — максимально возможное",
        as: Integer
...
begin
    ...
    $stderr.puts "\nВылетело по ошибке: #{e.message}. Смотрите логи"
end
